...
const ParticleText = ({
  ...
  useEffect(() => {
    ...
    const render = now => {
      ...
      pointer.smoothX += (pointer.x - pointer.smoothX) * 0.26;
      pointer.smoothY += (pointer.y - pointer.smoothY) * 0.26;
      ...
      particles.forEach(particle => {
        ...
        if (pointer.active && !reducedMotion && pointerRepel > 0 && repelRadius > 0) {
          ...
          if (distance > 0 && distance < repelRadius) {
            const force = Math.pow(1 - distance / repelRadius, 1.8) * pointerRepel;
    ...
    const sampleText = async () => {
      ...
      const maxParticles = Math.max(600, Math.min(2200, Math.floor((width * height) / 160)));
